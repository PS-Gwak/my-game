(() => {
  ...
  const IMAGE_ROLE = {
    TABLET: 0, // 1번 이미지: 오른쪽 미니 타블렛 한붓 기준선
    SKETCH: 1, // 2번 이미지: 메인 도화지 데생선 + 긁기 단계의 흰 종이 위 선화
  ...
  const TABLET_GUIDE_ALPHA = 0.24;
  const DRAW_LOOP_TARGET = 1;
  const FIXED_TABLET_CONTOUR_RATIOS = [
    [0.3123, 0.0974],
    [0.4470, 0.1565],
    [0.6243, 0.1586],
    [0.7602, 0.0872],
  ...
  const tabletCanvas = document.getElementById("tablet");
  const tabletCtx = tabletCanvas.getContext("2d");
  const TW = tabletCanvas.width;
  const TH = tabletCanvas.height;
  const TCELL = CONFIG.TABLET_CELL;
  const TGW = Math.ceil(TW / TCELL);
  const TGH = Math.ceil(TH / TCELL);
  const TABLET_GRID = { w: TW, h: TH, cell: TCELL, gw: TGW, gh: TGH };
  ...
  function tabletScale() {
    return Math.min(TW / W, TH / H);
  }

  function tabletHitRadius() {
    return Math.max(14, Math.round(CONFIG.HIT_RADIUS * tabletScale()));
  }

  function tabletWrongHitRadius() {
    return Math.max(9, Math.round(tabletHitRadius() * WRONG_HIT_RADIUS_RATIO));
  }

  function tabletLineWidth() {
    return Math.max(3.2, CONFIG.LINE_WIDTH * tabletScale());
  }
  ...
  function computeLayout() {
    ...
    tabletLayout = fitImageInGrid(images[IMAGE_ROLE.TABLET], TABLET_GRID);
  }

  function fitImageInGrid(img, grid) {
    ...
    return { x: (grid.w - w) / 2, y: (grid.h - h) / 2, w, h, cx: grid.w / 2, cy: grid.h / 2 };
  }
  ...
  function tabletPlaceFor() {
    return { x: tabletLayout.x, y: tabletLayout.y, w: tabletLayout.w, h: tabletLayout.h };
  }
  ...
  function buildContourNodes() {
    if (!tabletLayout || tabletLayout.w <= 0 || tabletLayout.h <= 0 || FIXED_TABLET_CONTOUR_RATIOS.length < 4) {
    ...
    contourNodes = FIXED_TABLET_CONTOUR_RATIOS.map(([ratioX, ratioY]) => ({
      x: tabletLayout.x + ratioX * tabletLayout.w,
      y: tabletLayout.y + ratioY * tabletLayout.h,
      kind: "outer",
      visited: false,
    }));
    assignStrokeOrder();
  ...
  function buildFallbackNodes() {
    const p = tabletLayout
      ? tabletPlaceFor()
      : { x: TW * 0.1, y: TH * 0.08, w: TW * 0.8, h: TH * 0.84 };
    const count = FIXED_TABLET_CONTOUR_RATIOS.length;
    contourNodes = [];
  ...
  function drawTargetCount() {
    return contourNodes.length ? contourNodes.length + 1 : 0;
  }

  function nodeIndexForPathStep(step) {
    if (!contourNodes.length || step < 0 || step >= drawTargetCount()) return -1;
    if (step === 0 || step === contourNodes.length) return 0;
    return contourNodes.length - step;
  }

  function hasPendingDrawTarget() {
    return nextPathStepIdx < drawTargetCount() && nextNodeIdx >= 0;
  }
  ...
  function resetCurrentDrawLoop({ clearTrace = true, resetBreaks = false } = {}) {
    clearWrongFlash();
    if (clearTrace) traceCtx.clearRect(0, 0, TW, TH);
  ...
  function startDraw() {
    ...
    ["chipDraw", "chipPaint", "chipDone"].forEach((id) => {
  ...
  function restartGame() {
    ...
    clearTimeout(autoPaintTimer);
    autoPaintTimer = null;
    clearTimeout(paintResultTimer);
    paintResultTimer = null;
    clearWrongFlash();
    wristMode = document.getElementById("wristToggle").checked;
    hovering = false;
    hoverSurface = null;
  ...
  function toSurfaceLocal(e, surface) {
    const targetCanvas = surface === "tablet" ? tabletCanvas : canvas;
    const targetW = surface === "tablet" ? TW : W;
    const targetH = surface === "tablet" ? TH : H;
    const rect = targetCanvas.getBoundingClientRect();
    const cx = (e.touches ? e.touches[0].clientX : e.clientX) - rect.left;
    const cy = (e.touches ? e.touches[0].clientY : e.clientY) - rect.top;
    return { x: cx * (targetW / rect.width), y: cy * (targetH / rect.height) };
  }

  function surfaceForStage() {
    if (stage === STAGE.DRAW) return "tablet";
    if (stage === STAGE.PAINT) return "tablet";
    return null;
  }

  function surfaceFromTarget(target) {
    if (target === tabletCanvas) return "tablet";
    if (target === canvas) return "main";
    return null;
  }

  function tryVisitContourAt(x, y) {
    if (!hasPendingDrawTarget()) return;
    if (wrongResetTimer) return;
    const hitRadius = tabletHitRadius();
    const wrongRadius = tabletWrongHitRadius();
    const targetIdx = nextNodeIdx;
    ...
    for (let i = 0; i < contourNodes.length; i++) {
      ...
      if (node.visited && !isTarget) continue;
  ...
  function visitNode(idx) {
    ...
    const pct = totalTargets ? (nextPathStepIdx / totalTargets) * 100 : 0;
    fade2 = getSketchRevealProgress();
    if (nextPathStepIdx >= totalTargets && pct >= CONFIG.DRAW_PASS && !drawCompleteQueued) {
  ...
  function drawTraceStroke(a, b) {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      traceCtx.lineWidth = tabletLineWidth() * (pass === 0 ? 0.92 : 0.55);
      traceCtx.beginPath();
  ...
  function scratchStroke(a, b) {
    const pTablet = tabletPlaceFor();
    const pMain = placeFor();
    if (!pTablet.w || !pTablet.h || !pMain.w || !pMain.h) return;
    const dist = Math.hypot(
      (b.x - a.x) * (pMain.w / pTablet.w),
      (b.y - a.y) * (pMain.h / pTablet.h)
    );
    ...
    for (let s = 0; s <= steps; s++) {
      ...
      const mainPt = mapPaintTabletPointToMain(tabletPt);
      if (mainPt) {
        scratchAt(mainPt.x, mainPt.y);
      }
    }
  }

  function mapPaintTabletPointToMain(point) {
    if (!tabletLayout || !layout) return null;
    const pTablet = tabletPlaceFor();
    const pMain = placeFor();
    const nx = (point.x - pTablet.x) / pTablet.w;
    const ny = (point.y - pTablet.y) / pTablet.h;
    if (nx < 0 || nx > 1 || ny < 0 || ny > 1) return null;
    return {
      x: pMain.x + nx * pMain.w,
      y: pMain.y + ny * pMain.h,
    };
  }
  ...
  function checkDrawHitsAlongStroke(from, to) {
    ...
    const steps = Math.max(1, Math.ceil(dist / Math.max(4, tabletHitRadius() * 0.45)));
    for (let s = 0; s <= steps; s++) {
      const t = s / steps;
  ...
  function onMove(e) {
    ...
    e.preventDefault();
    const p = toSurfaceLocal(e, surface);
    if (!lastPt) lastPt = p;
    applyInput(lastPt, p);
    lastPt = p;
  }
  ...
  function noteBreath() {
    if (strokeInProgress && lastNodeIdx >= 0 && hasPendingDrawTarget()) {
      breakCount++;
    }
    strokeInProgress = false;
  }
  ...
  function getSketchRevealProgress() {
    const currentLoop = drawCompleteQueued
      ? 0
      : (drawTargetCount() ? nextPathStepIdx / drawTargetCount() : 0);
  ...
  function updateTabletProgressLights() {
    const dot = document.getElementById("tabletDotDone");
    if (!dot) return;
    dot.classList.toggle("done", completedDrawLoops >= DRAW_LOOP_TARGET);
    dot.classList.toggle("active", stage === STAGE.DRAW && completedDrawLoops < DRAW_LOOP_TARGET);
  }
  ...
  function loop() {
    ...
    const drawPulse = stage === STAGE.DRAW && fade2 < 1 && hasPendingDrawTarget();
  ...
  function render() {
    ...
    tabletCtx.clearRect(0, 0, TW, TH);
    tabletCtx.fillStyle = "#f3ecda";
    tabletCtx.fillRect(0, 0, TW, TH);
    if (!layout || !tabletLayout) return;
    ...
    if (stage === STAGE.DRAW) {
      const sketchReveal = getSketchRevealProgress();
      fade2 = sketchReveal;
      if (sketchReveal > 0) {
        ctx.globalAlpha = sketchReveal;
    ...
    if (stage === STAGE.PAINT || stage === STAGE.RESULT) {
      renderTablet();
    }
  }
  ...
  function renderTablet() {
    ...
    const pTablet = tabletPlaceFor();
    if (stage === STAGE.DRAW) {
      tabletCtx.globalAlpha = TABLET_GUIDE_ALPHA;
      tabletCtx.drawImage(images[IMAGE_ROLE.TABLET], pTablet.x, pTablet.y, pTablet.w, pTablet.h);
      tabletCtx.globalAlpha = 1;
      tabletCtx.drawImage(traceLayer, 0, 0);
      drawContourNodes();
      return;
    }
    if (stage === STAGE.PAINT || stage === STAGE.RESULT) {
      tabletCtx.globalAlpha = 1;
      tabletCtx.drawImage(images[IMAGE_ROLE.COLOR], pTablet.x, pTablet.y, pTablet.w, pTablet.h);
      tabletCtx.globalAlpha = 1;
    }
  }
  ...
  function drawContourNodes() {
    const now = Date.now();
    const s = tabletScale();
    const visitedRadius = Math.max(2.8, 4.8 * s);
    const idleRadius = Math.max(4.4, 7.2 * s);
    const nextRadius = Math.max(4.8, 8 * s);
    const nextGlow = Math.max(9, 18 * s);
    const wrongRadius = Math.max(5, 8 * s);
    const wrongGlow = Math.max(10, 19 * s);
    for (let i = 0; i < contourNodes.length; i++) {
      const node = contourNodes[i];
      const isNext = i === nextNodeIdx && hasPendingDrawTarget();
      const isWrongFlash = i === wrongFlashNodeIdx && now < wrongFlashUntil;

      if (isWrongFlash) {
        const pulse = 0.65 + 0.35 * Math.sin(now / 34);
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, wrongGlow, 0, Math.PI * 2);
        tabletCtx.fillStyle = `rgba(230,48,35,${0.22 + pulse * 0.22})`;
        tabletCtx.fill();
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, wrongRadius, 0, Math.PI * 2);
        tabletCtx.fillStyle = "rgba(230,48,35,0.95)";
        tabletCtx.fill();
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, wrongRadius, 0, Math.PI * 2);
        tabletCtx.lineWidth = 2;
        tabletCtx.strokeStyle = "rgba(255,242,232,0.95)";
        tabletCtx.stroke();
      } else if (isNext) {
        // 다음에 이을 점은 목적지 표시등처럼 더 크게 빛난다.
        const pulse = 0.72 + 0.22 * Math.sin(now / 360);
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, nextGlow, 0, Math.PI * 2);
        tabletCtx.fillStyle = `rgba(245,167,0,${0.22 * pulse})`;
        tabletCtx.fill();
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, nextRadius, 0, Math.PI * 2);
        tabletCtx.fillStyle = `rgba(245,180,40,${0.82 * pulse + 0.16})`;
        tabletCtx.fill();
        tabletCtx.lineWidth = 1.7;
        tabletCtx.strokeStyle = "rgba(255,247,221,0.9)";
        tabletCtx.stroke();
      } else if (node.visited) {
        // 이미 이은 점 — 선을 탔다는 표시만 남긴다.
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, visitedRadius, 0, Math.PI * 2);
        tabletCtx.fillStyle = "rgba(104,78,36,0.58)";
        tabletCtx.fill();
      } else {
        // 아직 차례가 아닌 점도 또렷하게 보여서 전체 경로를 눈으로 잡을 수 있다.
        tabletCtx.beginPath();
        tabletCtx.arc(node.x, node.y, idleRadius, 0, Math.PI * 2);
        tabletCtx.fillStyle = "rgba(28,21,10,0.92)";
        tabletCtx.fill();
        tabletCtx.lineWidth = 1.2;
        tabletCtx.strokeStyle = "rgba(255,250,238,0.82)";
        tabletCtx.stroke();
      }
    }
  }
  ...
  function setChips(which) {
    ...
    if (which === "draw") d.classList.add("on");
    else if (which === "paint") p.classList.add("on");
    else if (which === "done") f.classList.add("on");
  }
  ...
  function setDebugSliderValue(sliderId, valId, value) {
    const slider = document.getElementById(sliderId);
    const valEl = document.getElementById(valId);
  ...
  const dbgControls = [
    ...
    wireSlider("dbgDrawPass", "dvDrawPass", "DRAW_PASS", 0, () => { if (stage === STAGE.DRAW) updateMeters(); }),
  ...
  setDebugSliderValue("dbgDrawPass", "dvDrawPass", CONFIG.DRAW_PASS);
  ...
  document.getElementById("dbgReset").addEventListener("click", () => {
    ...
    document.getElementById("dbgDrawPass").value = CONFIG.DRAW_PASS;
  ...
  tabletCanvas.addEventListener("mousedown", onDown);
  window.addEventListener("mousemove", onMove);
  window.addEventListener("mouseup", onUp);
  canvas.addEventListener("mouseenter", onEnter);
  canvas.addEventListener("mouseleave", onLeave);
  tabletCanvas.addEventListener("mouseenter", onEnter);
  tabletCanvas.addEventListener("mouseleave", onLeave);
  canvas.addEventListener("touchstart", onDown, { passive: false });
  canvas.addEventListener("touchmove", onMove, { passive: false });
  tabletCanvas.addEventListener("touchstart", onDown, { passive: false });
  tabletCanvas.addEventListener("touchmove", onMove, { passive: false });
