...
(function () {
  ...
  const HOST_LINES = {
    ...
    silence_ment: [
      ...
      () => `📢 경매장 전체가 숨을 죽이고 있군요.`,
      () => `📢 잠시 정적이 흐르네요.`,
      (ctx) => `📢 다들 ${ctx.prevBidderLabel}만 보고 있습니다.`,
      () => `📢 고요합니다… 누가 이 침묵을 깰까요?`,
  ...
  function bidderDisplayName(id) {
    if (id === 'user') return (typeof window !== 'undefined' && window.playerName) ? window.playerName : '나';
  ...
  function makeCpus(rng, round, isExtreme, mean) {
    ...
    const names = rng.shuffle(CONFIG.cpuNamePool.slice());
  ...
  function pickHostLineFromIndexes(keyGroup, indexes, ctx) {
    const pool = HOST_LINES[keyGroup];
    if (!pool || pool.length === 0) return null;
    const candidates = indexes.map(i => pool[i]).filter(Boolean);
    if (candidates.length === 0) return pickHostLine(keyGroup, ctx);
    return state.rng.pick(candidates)(ctx);
  }
  ...
  const PERSUASION_GENERIC_IDX = [1, 2, 3, 4, 6, 7];
  ...
  //   meta = { cat, cpuId, leaderAtIssue } — 큐 배출 직전 검증 게이트(drainHostQueue)에서 사용.
  ...
  function makeFixedPrice(ctx) {
    ...
    const api = {
      ...
      _userBuy() {
        ...
        winner = 'user';
        setLastBidder('user');
        const _playerName = (typeof window !== 'undefined' && window.playerName) ? window.playerName : '나';
      ...
      const api = {
        ...
        _hostCheck(dt) {
          ...
          if (bidCount > 0
            ...
            let line;
            if (remaining1v1 && chaserIsActive() && rng.next() < 0.5) {
              // v11.04 확정안 MSG-5: 지목 추격자 대상 — chaser_more 풀.
              line = pickHostLine('chaser_more', fmtCtx(null));
            } else {
              line = pickPersuasionGeneric(fmtCtx(null));
            }
            if (line) logBid(line, 'host');
          ...
          if (bidCount > 0
            ...
            const tline = pickHostLine('take_your_time', fmtCtx(null));
            if (tline) logBid(tline, 'host');
            return false;
          }
          ...
          if (bidCount > 0
            ...
            const sline = pickHostLine('silence_ment', fmtCtx(null));
            if (sline) logBid(sline, 'host');
          ...
          if (nextHostShoutMs <= 0) {
            ...
            if (bidCount === 0) {
              ...
              // 멘트 정밀화 P2: 추격자(prevBidder) 있을 때만 도발, 없으면 이름 없는 호가 안내.
              line = (r < 0.4 && chaserIsActive())
              ...
              // 추격자 없거나 retired면 이름 없는 cooldown_user 풀로 대체.
              line = chaserIsActive()
                ? pickHostLine(r < 0.4 ? 'counter_cpu' : 'cooldown_cpu', fmtCtx(null))
                : pickHostLine('cooldown_user', fmtCtx(null));
            } else {
              line = pickHostLine('cooldown_user', fmtCtx(null));
            }
        ...
        step(dtMs) {
          ...
          const dt = scaledDt(dtMs);
          totalElapsedMs += dt;
          // v11.04 코드리뷰 2차 #3: 마무리 시퀀스 진행 중이라도 CPU·유저 입찰을 막지 않는다.
          //   시퀀스는 박자대로 진행하되, 입찰이 들어오면 _placeBid가 _cancelClosingSeq로 마감을 취소한다.
          //   시퀀스 중에는 host 잡담(_hostCheck)·cooldown 만료 재진입은 건너뛰고 입찰 스캔만 돌린다.
          const inClosingSeq = (closingSeqStep >= 0);
          if (inClosingSeq) {
            this._runClosingSeq(dt);
            if (over) return;
            // 시퀀스가 입찰로 취소됐을 수 있으니 다시 확인 — 취소됐으면 아래 일반 흐름으로 진행.
          }
          updateFlowStage();
          ...
          // v11.04 코드리뷰 2차 #3: 시퀀스 진행 중에는 host 잡담·cooldown 만료 재진입을 건너뛴다(이미 cooldown<=0이라 재트리거 위험).
          //   대신 아래 CPU·sniper·WTH 입찰 스캔은 그대로 돌려 막판 입찰을 받는다.
          if (!inClosingSeq) {
            hostCooldownMs -= dt;
            if (hostCooldownMs <= 0) {
              // cooldown 만료 → 종료.
              // v11.04 확정안 MSG-4: 입찰이 있었으면(낙찰 가능) 바로 끝내지 않고 마무리 정형 시퀀스 시동.
              if (topBidder != null && closingSeqStep < 0) {
                closingSeqStep = -1;
                closingSeqNextAtMs = 0;
                // 시퀀스 박자가 큐에 남은 옛 멘트와 섞이지 않게 큐 비움.
                state.hostQueue = [];
                this._runClosingSeq(0);  // 첫 줄(경고) 즉시
                return;
              }
              over = true;
              hostCooldownMs = 0;
      ...
      const api = {
        ...
        _hostCheck(dt) {
          ...
          if (bidCount > 0
            ...
            if (remaining1v1 && chaserIsActive() && rng.next() < 0.5) {
              // v11.04 확정안 MSG-5: 지목 추격자 대상 — chaser_more 풀.
              pline = pickHostLine('chaser_more', fmtCtx(null));
            } else {
              pline = pickPersuasionGeneric(fmtCtx(null));
            }
          ...
          if (nextFlavorMs <= 0) {
            ...
            let line = null;
            if (lastId === 'user') {
              // 멘트 정밀화 P2: 추격자 있을 때만 도발.
              line = chaserIsActive()
                ? pickHostLine('counter_user', fmtCtx(null))
                : pickHostLine('cooldown_user', fmtCtx(null));
            } else if (typeof lastId === 'number') {
              // 멘트 정밀화 P2: counter/cooldown_cpu는 추격자(prevBidder) 대상. 없으면 이름 없는 풀.
              line = chaserIsActive()
                ? pickHostLine(rng.next() < 0.4 ? 'counter_cpu' : 'cooldown_cpu', fmtCtx(null))
                : pickHostLine('cooldown_user', fmtCtx(null));
            } else {
              line = pickHostLine('cooldown_user', fmtCtx(null));
            }
  ...
  function renderCpus() {
    ...
    for (const c of state.cpus) {
      ...
      card.innerHTML = `
        <div class="cpu-num">${cpuDisplayName(c)} <span class="cpu-persona-emoji">${personaEmoji}</span></div>
  ...
  function highlightNamesInText(text) {
    ...
    // 플레이어 이름 청록으로 — 호명 자리(앞·뒤가 한글/영문/숫자가 아닌 경계)에서만 매치.
    // 비유: '나'는 한국어에서 너무 흔한 음절이라 '나올까요'·'나가다'·'어쩌나' 같은 단어 안쪽까지 강조되면 잘못됨.
    // 그래서 양쪽이 한글 음절(가-힣) 또는 자음·모음(ㄱ-ㅎㅏ-ㅣ)·영문·숫자가 아닐 때만 진짜 호명으로 본다.
    const pname = (typeof window !== 'undefined' && window.playerName) ? window.playerName : '나';
    if (pname) {
  ...
  function renderBidLog() {
    ...
    for (const e of recent) {
      ...
      // 디렉터 사이클 #5: 이름 강조용으로 innerHTML 사용 (escape 처리 됨)
  ...
  function renderCpuLedger() {
    ...
    for (const id of sortedIds) {
      ...
      const name = id === 'user' ? `👤 ${(typeof window !== 'undefined' && window.playerName) ? window.playerName : '나'}` : cpuDisplayName(id);
  ...
  function renderPaintingsLedger() {
    ...
    for (const r of state.cycleResults) {
      ...
      if (!r.purchased || r.winner == null) {
        ...
        const name = bidderDisplayName(r.winner);
  ...
  function showRoundResult() {
    ...
    else winnerLabel = bidderDisplayName(r.winner);
