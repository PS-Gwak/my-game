...
const state = {
  ...
  savePointNewlyCreated: false,           // v4.1 — 이번 굴림에 세이브가 null → 값으로 새로 생긴 경우만 true
...
function takeSnapshot() {
  return {
    ...
    savePointNewlyCreated: state.savePointNewlyCreated, // v4.1
...
function restoreSnapshot(snap) {
  ...
  state.savePointNewlyCreated = snap.savePointNewlyCreated; // v4.1
...
async function rollDice(isFromReroll) {
  ...
  // v4.1 — 매 굴림마다 "이번 굴림에 세이브가 새로 생겼는지" 플래그 초기화.
  // maybeUpdateSavePoint가 굴림 중 null → 값으로 새로 만들면 true 로 켬.
  state.savePointNewlyCreated = false;
...
async function applyMovement(val) {
  ...
  if (savedAtSavePoint) {
    // v4.1 — 화면 가운데 토스트 제거 (이미 지나친 세이브로 음수 굴림 돌아올 때마다 뜨는 게 게임 방해).
    // 우측 이벤트 로그에만 한 줄 남김.
 ...
 *  v4.1 — 세이브가 없다가 새로 생긴 경우만 savePointNewlyCreated=true 로 표시.
 *  9-2 선택지는 이 플래그가 true 일 때만 발동(이미 보상 받은 자리로 함정 후 돌아오는 흐름) */
function maybeUpdateSavePoint(tilePos) {
  if (!SAVE_TILES.includes(tilePos)) return;
  if (state.savePoint == null) {
    state.savePoint = tilePos;
    state.savePointNewlyCreated = true;
    play('upgradeClick');
...
async function handleTileReward(pos) {
  ...
  if (!state.tileRewardClaimed.has(pos)) {
    ...
    // 이미 받음 — 인라인 선택 패널 (v4.1: 세이브가 *이번 굴림에 새로 생긴 경우*에만)
    // 즉 함정 빠진 뒤 다시 그 자리 도달해서 세이브가 null → 값으로 생긴 순간만 발동.
    // 단순 음수 굴림으로 같은 자리 들렀다 오는 경우엔 발동 안 함.
    if (!SAVE_TILES.includes(pos)) return; // 3번 등은 재선택 안 함
    if (!state.savePointNewlyCreated) return; // v4.1 — 핵심 조건 좁힘
